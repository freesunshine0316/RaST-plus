...
class DataLoader(object):
    def __init__(self, data_dir, bert_class, params, tag_pad_idx=-1):
        ...
        self.tokenizer = transformers.AutoTokenizer.from_pretrained(bert_class, do_lower_case=False)
    ...
    def _split_to_wordpieces_span(self, tokens, label_action, label_start, label_end):
        ...
        for i, token in enumerate(tokens):
            ...
            if token in self.special_tokens:
                ...
                filtered_pieces = []
                for piece in pieces:
                    if piece == XLM_ROBERTA_SPACE:
                        continue
                    elif XLM_ROBERTA_SPACE in piece:
                        piece = piece.replace(XLM_ROBERTA_SPACE, '')
                    else:
                        pass
    ...
    def load_sentences_tags(self, sentences_file, tags_file, d, with_ref=False, loading_len=-1, using_ernie=False):
            ...
            with open(sentences_file, 'r') as file1:
                with open(tags_file, 'r') as file2:
                    for line1, line2 in zip(file1,file2):
                        src, tgt = line1.split("\t")
                        src = src.replace('[SEP]', self.tokenizer.sep_token)
                        if using_ernie:
                            src = src.replace('|', '[SEP1]')
                            src = src.replace('*', '[SEP2]')
                        ...
                        if len(bert_tokens) >= 500:
                            print('!!! Filtering over-size instance')
                            continue
                        ...
                        try:
                            if not using_ernie:
                                ...
                                boundaries.append(bert_tokens.index('[SEP1]'))
                        except:
                            print(bert_tokens)
                            raise
                        ...
                        assert len(sentences[-1][0]) >= len(action[-1])
                        processed_len += 1
                        if loading_len > -1 and processed_len == loading_len:
                            break
    ...
    def load_data(self, data_type, fold='0', data_path=None, with_ref=False, loading_len=-1, hard=False, using_ernie=False):
        ...
        if data_type in ['train', 'dev', 'test', 'unseen_test']:
            if not hard:
                print('Loading ' + data_type)
                sentences_file = '.'.join([self.data_dir, data_type+"_"+fold, 'sentences.txt'])
                print(f'Loading from {sentences_file}')
                tags_path = '.'.join([self.data_dir, data_type+'_'+fold, 'tags.txt'])
            else:
                print('Loading ' + 'hard ' + data_type)
                sentences_file = '.'.join(['data/pretrain', data_type, 'hard','sentences.txt'])
                tags_path = '.'.join(['data/pretrain', data_type, 'hard', 'tags.txt'])
            self.load_sentences_tags(sentences_file, tags_path, data, with_ref=with_ref, loading_len=loading_len, using_ernie=using_ernie)
...
def split_train_into_train_dev(train_data, dev_size):
    new_dev_data = {}
    new_train_data = {}
    for key in ['action', 'start', 'end', "ref", 'data']:
        new_dev_data[key] = train_data[key][:dev_size]
        new_train_data[key] = train_data[key][dev_size:]

    new_dev_data['size'] = dev_size
    new_dev_data['query_boundary'] = train_data['query_boundary']
    new_train_data['size'] = train_data['size'] - dev_size
    new_train_data['query_boundary'] = train_data['query_boundary']
    return new_train_data, new_dev_data

def merge_trains_into_train(clean_data, noisy_data, noisy_portion=0.):
    new_train_data = {}
    new_train_data['size'] = clean_data['size'] + noisy_data['size']
    if noisy_portion != 0:   # 0 means all noisy data
        mixing_noisy_num = int(len(clean_data['data']) * noisy_portion)
        assert mixing_noisy_num <= len(noisy_data['data'])
        new_train_data['size'] = clean_data['size'] + mixing_noisy_num
        sampled_noisy_data_indices = random.sample(range(noisy_data['size']), mixing_noisy_num)
        sample_noisy_data = {}
        for key in ['action', 'start', 'end', "ref", 'data', 'query_boundary']:
            sample_noisy_data[key] =[noisy_data[key][i] for i in sampled_noisy_data_indices]
        noisy_data = sample_noisy_data

    for key in ['action', 'start', 'end', "ref", 'data', 'query_boundary']:
        new_train_data[key] = clean_data[key] + noisy_data[key]
