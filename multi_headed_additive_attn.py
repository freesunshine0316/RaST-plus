...
class Additive_Attention(nn.Module):
    ...
    def forward(self, queries, keys, values, mask):
        ...
        combined = torch.tanh(self.linear_concat(torch.cat((q, k), dim=-1))) # [bs, 1, len_q, len_k, size]
        ...
        return logits.squeeze(dim=1) # [bs, seq, seq]
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, key, value, query, mask=None, type=None):
        ...
        key = shape(key) # [batch, 1, seq, dim]
        value = shape(value) # [batch, 1, seq, dim]
        ...
        query = shape(query) # [batch, 1, seq, dim]
